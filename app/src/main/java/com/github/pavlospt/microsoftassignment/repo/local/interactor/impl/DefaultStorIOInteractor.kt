package com.github.pavlospt.microsoftassignment.repo.local.interactor.impl
...
import com.github.pavlospt.microsoftassignment.repo.local.storio.BeersTable.Companion.GET_ALL_BEERS_QUERY
...
class DefaultStorIOInteractor @Inject constructor(
  ...
  override fun search(terms: List<String>): Observable<List<BeerStorIOModel>> {
    ...
    val query = Query.builder().table(
        BeersTable.TABLE_NAME)
  ...
  * */
  override fun clearBeers(): Observable<DeleteResult> {
    return storIOSQLite
        .delete()
        .byQuery(
            BeersTable.DELETE_ALL_BEERS_QUERY)
        .prepare()
        .asRxObservable()
        .subscribeOn(Schedulers.io())
  }
